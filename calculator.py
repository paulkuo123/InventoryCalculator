...
class SalesCalculator(QWidget):
    ...
    def initUI(self):
        ...
        # 計算按鈕
        self.calc_button = HoverButton('計 算 預 期 庫 存')
        ...
        QShortcut(QKeySequence("Return"), self, self.calc_button.click)
    ...
    def calculate(self):
        try:
            ...
            # 特殊情況：當庫存為 0 且月銷量也為 0 時，使用歷史佔比法
            if current_inventory == 0 and monthly_sales == 0:
                ...
                total_monthly_sales, ok = QInputDialog.getDouble(
                    self, "輸入數據", "檢測到庫存與月銷量皆為 0，請輸入商品整體總月銷量：",

...
給庭安一次看一案的靜態審核工作台。只讀 AI-4 有候選的 69 筆＋Plan C 已擱高／中 7 筆附錄。**沒改 `golden_table.json`、沒合 main、沒加採購車。**
...
| 區塊 | 筆數 | 說明 |
|---|---:|---|
| 主佇列必審 | **69** | AI-4 `with_candidate`；高 14／中 18／低 37 |
| 附錄 `shelved_round1` | **7** | Plan C 已擱高 2／中 5，**不是**本批必審 |
| 未進主佇列 | — | AI-4 `blocked_captcha`、`no_candidate` 都沒塞進來 |

排序：**高 → 中先、應補量大的排前面**；低信心預設不在第一屏（切「低」分頁才看）。預設分頁是「優先（高／中）」，會跳到第一筆還沒審的。

主佇列裡有 2 筆 AI-4 標過 `shelved_round1`（貼貼奶狗、發芽碳球），仍算在 69 裡才對得上高／中／低加總；畫面上有「已擱 round1」標籤，庭安可略過。完整 7 筆已擱另放附錄分頁。
...
1. 先把「優先（高／中）」32 筆對圖（含後背包白應補 400、清潔筆、軍規／蝴蝶結殼）。

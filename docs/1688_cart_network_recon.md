...
**2026-09-17 live 已確認加車 `addcargo` 與改量 Ultron `astoreservice.async`**（一筆 offer／一筆 sku；未結算、未刪列、未付款）。`TODO_live` 對這兩條已清除。sku-selector 精確 path 仍是 candidate。本 PR **不**實作 mutate HTTP client。
...
# 可選：另開 1 個 offer 分頁（仍不點加車）。watch 期間操作者可在詳情頁手動點 1 sku、或在車頁改 1 列數量。
...
下列 URL：讀車來自 freeze 白名單；**加車／改量為 2026-09-17 一筆 live 抓包確認**（樣本 offer／sku／cartId 僅供對照，不要當全域常數）。sku-selector 精確 path 仍是 candidate。
...
| sku_selector（加車輔助） | **candidate** | `wosc.queryofferskuselectormodel` | GET jsonp 或 POST — 精確 path 仍待補 | `referer` 商品頁；`cookie` 遮罩 | `data.skuSelectorBizModel.skuInfoMap[skuId]`：`skuId, specId, specAttrs, price, canBookCount` |
...
改量當下也常伴隨：`asyncload`（頁面片段）、tracking `cart_goods_number_edit`。分類時 `.asyncload`／`.render` 仍是 `read_cart`；只有帶 Ultron `operator=item_{cartId}` 且 `fields.quantity` 的 `.async` 才是 `change_qty`。
...
5. 分類器應標出 `addcargo` 與 Ultron `astoreservice.async`。仍不要在本 PR 寫 production client。mtop sign／cookie／Ultron compress 尚未逆向成可重放客戶端。
...
不需 Playwright、不需 1688。Fixture 在 `tests/fixtures/1688_cart_network/sample_capture.json`（合成包、無真實 cookie；形狀對齊 confirmed addcargo + Ultron async）。

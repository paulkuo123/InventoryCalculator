    ...
    try:
        ...
        try:
            ...
            if filter_checkbox.count() > 0:
                ...
                # Dashboard 應與目前顯示的需補貨型號同步；若沒有型號被隱藏，數字可維持不變。
                if after_stock <= original_stock:
                    tracker.add_test("index.html", "filterMode Dashboard 同步篩選", f"正確（{original_stock} -> {after_stock}）")
                else:
                    tracker.add_bug("index.html", f"filterMode 讓 Dashboard 庫存增加：從 {original_stock} -> {after_stock}", "high")
        except Exception as e:

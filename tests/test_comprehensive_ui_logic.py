...
def test_dashboard_statistics():
    ...
    if script_js_path.exists():
        ...
        stats_start = content.find("function calculateInventoryStatistics")
        dashboard_start = content.find("// 更新儀表板UI", stats_start)
        stats_content = content[stats_start:dashboard_start]
        if "advancedKeyword && advancedKeyword.trim()" in stats_content and \
           ("filteredProducts = filtered" in stats_content or "filteredProducts[productId]" in stats_content):
        ...
        # 檢查：Dashboard 是否和商品列表使用相同的 filterMode？
        if "const filterMode" in stats_content and "if (filterMode)" in stats_content:
            report.add_pass("Dashboard 遵循 filterMode - 統計與目前可見型號一致")
        else:
            report.add_bug(
                "Dashboard 可能沒有依 filterMode 統計目前可見型號",
                severity="high"
            )
